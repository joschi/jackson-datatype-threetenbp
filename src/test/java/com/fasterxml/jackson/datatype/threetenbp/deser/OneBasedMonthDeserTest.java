...
public class OneBasedMonthDeserTest extends ModuleTestBase
{
    ...
    public void testBadDeserializationAsString01_oneBased() {
        assertError(
            ...
            "Cannot deserialize value of type `org.threeten.bp.Month` from String \"notamonth\": not one of the values accepted for Enum class: [OCTOBER, SEPTEMBER, JUNE, MARCH, MAY, APRIL, JULY, JANUARY, FEBRUARY, DECEMBER, AUGUST, NOVEMBER]"
        );
    }
    ...
    public void testDeserializationWithTypeInfo01_oneBased() throws Exception
    {
        ObjectMapper MAPPER = new ObjectMapper()
            .registerModule(new ThreeTenTimeModule().enable(ThreeTenTimeFeature.ONE_BASED_MONTHS));
        MAPPER.addMixIn(TemporalAccessor.class, MockObjectConfiguration.class);

        TemporalAccessor value = MAPPER.readValue("[\"org.threeten.bp.Month\",11]", TemporalAccessor.class);
    ...
    private ObjectReader readerForZeroBased() {
        return JsonMapper.builder()
                .addModule(new ThreeTenTimeModule()
                        .disable(ThreeTenTimeFeature.ONE_BASED_MONTHS))
                .build()
